      ...
      (err, row) => {
        ...
        if (row.password !== body.password || row.intentos >= 3) {
          conn.serialize(() => {
            conn
              .run(
                `update usuarios set intentos=intentos+1 where usuario='${body.usuario}';`,
                (errRUN) => {
                  if (errRUN) {
                    console.log(errRUN.message, 'Line : 125');
                    return;
                  }
                }
              )
              .get(
                `select intentos from usuarios where usuario='${body.usuario}'`,
                (errGET, rowGet) => {
          ...
          conn.run(
            `update usuarios set intentos=0 where usuario='${body.usuario}';`,
            (errRUN) => {
              if (errRUN) {
...
async function register(body, callback) {
  try {
    ...
    conn.all(
      ...
      (err, rows) => {
        if (err) {
          console.log('Inside all line:200', err.message);
          return;
        }
        if (rows[0]) {
          callback({
        ...
        `
            ...
            .get(
              ...
              (err, row) => {
                ...
                console.log(row);
                if (err) {
                  console.log('Inside get line:226', err.message);
                  console.log(err);
                  return;
                }
    ...
    conn.get(`select * from usuarios where id=${body.id}`, (err, row) => {
      ...
      if (typeof row === 'undefined') {
        ...
        conn.serialize(() => {
          conn
            .run(
              body.usuario && body.usuario.length > 0
                ? `update usuarios set nombre='${body.nombre}' where id=${body.id}`
                : '',
              (e) => {
            ...
            .run(
              body.password.length > 0
                ? `update usuarios set password='${body.password}' where id=${body.id}`
                : '',
              (e) => {
            ...
            .run(
              body.respuesta1.length > 0
                ? `update usuarios set pregunta1=${body.pregunta1} where id=${body.id}`
                : '',
              (e) => {
                if (!e) {
                  console.log('Updated Pregunta1');
                  conn.run(
                    `update usuarios set respuesta1=${body.respuesta1} where id=${body.id}`
                  );
            ...
            .run(
              body.respuesta2.length > 0
                ? `update usuarios set pregunta2=${body.pregunta2} where id=${body.id}`
                : '',
              (e) => {
                if (!e) {
                  console.log('Updated Pregunta2');
                  conn.run(
                    `update usuarios set respuesta2=${body.respuesta2} where id=${body.id}`
                  );
            ...
            .get(
              `select * from usuarios where id=${body.id}`,
              (errGet, newRow) => {
                if (errGet) {
  ...
  try {
    conn.get(
      ...
      (err, row) => {
        ...
        conn.run(
          `update usuarios set password='${body.password}' where usuario='${body.usuario}'`,
          (err) => {
            if (err) {
              console.log(err);
              return;
              ...
              conn.run(
                ...
                (errRUN) => {
                  if (errRUN) {
                    console.log(errRUN.message, 'Line : 310');
                    return;
                  }
                }
              );
              ...
              if (body.withUser)
                ...
                callback({ response: `Contraseña cambiada de forma exitosa` });
              return;
            }
          }
        );
...
async function getQuestions(usuario, callback) {
  try {
    conn.get(
      `select \`usuario\`,\`pregunta1\`,\`pregunta2\` from usuarios where usuario='${usuario}'`,
      (err, row) => {
        if (typeof row === 'undefined') {
          callback({
        ...
        //Q de questions
        conn.all(
          `select * from preguntas where id=${row.pregunta1} or id=${row.pregunta2}`,
          (err, rows) => {
            callback({
...
async function checkQuestions(body, callback) {
  try {
    conn.get(
      ...
      (err, row) => {
        if (
          row.respuesta1.toLowerCase() === body.respuesta1.toLowerCase() &&
          row.respuesta2.toLowerCase() === body.respuesta2.toLowerCase()
        ) {
...
async function makeReg(body) {
  try {
    ...
    conn.run(
      `insert into registros(\`user\`,\`archivo\`,\`accion\`,\`fecha\`) values(${body.userId}, '${path}', '${body.accion}', date())`,
      (err) => {
        if (err) {
          console.log(err);
          return;
...
async function getUsers(callback) {
  conn.all(
    `select \`id\`,\`nombre\`,\`desde\`,\`usuario\`,\`pregunta1\`,\`pregunta2\`,\`respuesta1\`,\`respuesta2\`,\`nivel\` from usuarios;`,
    (err, rows) => {
      if (err) {
        console.log(err);
        return;
...
function userExists(id, callback) {
  try {
    conn.get(`select usuario from usuarios where id=${id};`, (err, row) => {
      if (err) {
        console.log(err);
        return;
      }
...
function setNivel(body, callback) {
  try {
    conn.run(
      `update usuarios set nivel=${body.newNivel} where id=${body.id};`,
      (err) => {
        if (err) {
          console.log(err);
          return;
...
function deleteUser(id, callback) {
  ...
  conn.serialize(() => {
    conn
      .get(`select usuario from usuarios where id=${id}`, (err, row) => {
        if (err) {
          console.log(err);
          callback({ response: 'wrong' });
          return;
        }
        usuario = row.usuario;
      })
      .run(`delete from usuarios where id=${id}`, (err) => {
        if (err) {
          console.log(err);
          callback({ response: 'wrong' });
          return;
