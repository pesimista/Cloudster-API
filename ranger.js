...
function getFile(id) {
  if (typeof id === 'string')
    return fileList.find((f) => f.id === parseInt(id));
...
function getFilesInDirectory(realativeDir) {
  ...
  unprocessedFiles.forEach((dirent, index) => {
    ...
    try {
      ...
      console.log(
        `ERROR : no such file or directory, stat ${dir + dirent.name}`
      );
      return '404 not found';
    }
...
function loadAllFiles(index, dir, nivel) {
  ...
  unprocessedFiles.forEach((dirent) => {
    ...
    if (
      ...
      dirent.name !== 'build' &&
      dirent.name !== 'files.json' &&
      dirent.name !== 'ranger.json'
    ) {
      try {
        stat = fs.statSync(dir + '\\' + dirent.name);
      } catch (e) {
        console.log(`no such file or directory, stat ${dir + dirent.name}`);
        return '404 not found';
      }
      Files.push({
        id: ++index,
      ...
      if (!dirent.isFile()) {
        let dependedFiles = loadAllFiles(
          index,
          dir + '\\' + dirent.name,
          nivel
        );
...
function createJSON(data, message, con) {
  fs.writeFile('files.json', data, 'utf8', (err) => {
    if (err) {
...
function addToFile(dir, index, dep) {
  ...
  try {
    ...
    console.log(`no such file or directory, stat ${dir}`);
    return '404 not found';
  }
  fileList.push({
    id: index,
...
module.exports.getFiles = getFilesInDirectory;
module.exports.getAllFiles = getAllFiles;
module.exports.getDependency = getDependency;
module.exports.getFile = getFile;
module.exports.wd = cwd;
